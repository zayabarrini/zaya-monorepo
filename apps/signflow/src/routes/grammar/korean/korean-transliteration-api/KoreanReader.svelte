<script lang="ts">
  ...
  $: currentParagraphText = currentParagraph?.ko || "";
  ...
  function getParticleLabel(
    ...
    if (analysis.particle_type === "NONE") return "";
    return analysis.particle_type.toLowerCase();
  }

  function getVerbFormLabel(
    analysis: WordAnalysis
  ): string {
    if (analysis.verb_form === "NONE") return "";
    return analysis.verb_form.toLowerCase();
  }
...
{#if loading}
  <div class="loading-container">
    ...
    <p>Loading your Korean reader...</p>
  </div>
...
{:else if sections.length > 0}
  <div class="korean-reader">
    <!-- Beautiful Header - Korean Theme -->
    <header class="reader-header">
      <div class="header-content">
        <h1>한국어 리더</h1>
        <p class="subtitle">
          Korean Reader with Grammar Analysis
        </p>
      </div>
          ...
          <div class="settings-section">
            ...
            <div class="fields-grid">
              <label class="checkbox-label field-item">
                ...
                <span class="field-badge">Hangul text</span>
              </label>
              <label class="checkbox-label field-item">
                <input
                  type="checkbox"
              ...
              <label class="checkbox-label field-item">
                <input
                  type="checkbox"
                  bind:checked={
                    displayConfig.showDecomposition
                  }
                />
                <span class="checkbox-custom"></span>
                <span class="field-name">Decomposition</span
                >
                <span class="field-badge"
                  >Jamo breakdown</span
                >
                  ...
                  {#each currentParagraph.analysis as analysis (analysis.word + analysis.transliteration)}
                    ...
                    {:else}
                      ...
                      >
                        ...
                        {#if displayConfig.showParticleType && analysis.particle_type !== "NONE"}
                        ...
                        {#if displayConfig.showVerbForm && analysis.verb_form !== "NONE"}
                        ...
                        {#if displayConfig.showHonorificLevel && analysis.honorific_level !== "PLAIN"}
                        ...
                        {#if displayConfig.showDecomposition && analysis.decomposition && Object.keys(analysis.decomposition).length > 0}
                          <span class="decomposition-field">
                            {#each Object.entries(analysis.decomposition) as [char, parts]}
                              <span
                                class="decomposition-item"
                              >
                                {char}: {parts.initial}+{parts.medial}{parts.final
                                  ? "+" + parts.final
                                  : ""}
                              </span>
                            {/each}
    ...
    <div class="legend">
      ...
      <div class="legend-items">
        ...
        >
        <span class="legend-item" data-category="NUMERAL"
          >Numeral</span
        >
      </div>
    </div>
...
<style>
  ...
  * {
  ...
  /* Word Container */
  .korean-reader :global(.word-container) {
    display: inline-flex;
  ...
  .korean-reader :global(.word-container:hover) {
  ...
  /* Word field */
  .korean-reader :global(.word-field) {
    font-size: 2rem;
    font-weight: 600;
    color: #2c3e50;
    font-family: "Malgun Gothic", "맑은 고딕", sans-serif;
  ...
  .korean-reader :global(.transliteration-field) {
    font-size: 0.85rem;
    color: #666;
    font-style: italic;
  ...
  /* Translation field */
  .korean-reader :global(.translation-field) {
    font-size: 0.8rem;
    color: #2c3e50;
  ...
  /* Metadata fields */
  .korean-reader :global(.pos-field),
  .korean-reader :global(.syntax-field),
  .korean-reader :global(.particle-field),
  .korean-reader :global(.verb-form-field),
  .korean-reader :global(.honorific-field),
  .korean-reader :global(.semantic-field),
  .korean-reader :global(.decomposition-field) {
    font-size: 0.65rem;
  ...
  .korean-reader :global(.decomposition-item) {
    display: inline-block;
  ...
  .korean-reader :global(.word-noun) {
    background: linear-gradient(135deg, #f1f8e9, #dcedc8);
    border-color: #8bc34a;
  }

  .korean-reader :global(.word-verb) {
    background: linear-gradient(135deg, #ffebee, #ffcdd2);
    border-color: #ef5350;
  }

  .korean-reader :global(.word-adjective) {
    background: linear-gradient(135deg, #f3e5f5, #e1bee7);
    border-color: #ab47bc;
  }

  .korean-reader :global(.word-adverb) {
    background: linear-gradient(135deg, #e0f7fa, #b2ebf2);
    border-color: #26c6da;
  }

  .korean-reader :global(.word-pronoun) {
    background: linear-gradient(135deg, #efebe9, #d7ccc8);
    border-color: #8d6e63;
  }

  .korean-reader :global(.word-particle) {
    background: linear-gradient(135deg, #fff3e0, #ffe0b2);
    border-color: #ffa726;
  }

  .korean-reader :global(.word-conjunction) {
  ...
  .legend-item[data-category="NUMERAL"] {
    background: #e0f7fa;
    border-color: #00bcd4;
  }
