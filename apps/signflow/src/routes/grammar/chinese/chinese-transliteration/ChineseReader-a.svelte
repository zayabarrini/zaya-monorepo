<script lang="ts">
  ...
  function buildAnnotatedHTML(
    ...
    return annotations
      .map((ann) => {
        if (ann.type === "punct") {
          return `<span class="punctuation-token">${ann.content}</span>`;
        } else {
          const syntaxClass = ann.syntax || "";
          const syntaxLabel =
            showSyntaxLabels && syntaxClass
              ? `<span class="syntax-label">${syntaxClass}</span>`
              : "";
          const grammaticalClass = ann.grammatical_class
            ? `<span class="grammatical-class">${ann.grammatical_class}</span>`
            : syntaxLabel;

          return `<ruby class="chinese ruby-${syntaxClass}" data-syntax="${syntaxClass}">
            ${grammaticalClass}
            <span class="word-token">${ann.word || ""}</span>
          ...
          <div class="paragraph-card">
            ...
            {#if showCleanVersion}
              <div class="clean-version">
                <!-- <span class="language-label">ZH</span> -->
            ...
            <!-- Annotated Version -->
            {#if showAnnotations && currentParagraph.annotations.length > 0}
              <div class="transliterated-version">
                <!-- <span class="language-label">ZH +</span> -->
