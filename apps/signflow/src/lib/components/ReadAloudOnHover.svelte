<script lang="ts">
  ...
  function setReadingPosition(event) {
    ...
    console.log(isSpeakingMode);
    if (isSpeakingMode) {
      speak();
    }
  }

  // Hover rest handler - reads word after hovering for 500ms
  function handleHoverRest(event: MouseEvent) {
    ...
    // Set new timeout to read the word
    hoverTimeout = setTimeout(() => {
      if (
        ...
        // Get the word from the hovered element
        const hoveredText = target.innerText?.trim();
        if (hoveredText) {
          text = hoveredText;
          speak();
        }
...
<svelte:window
  on:click={setReadingPosition}
  on:mousemove={handleHoverRest}
/>
