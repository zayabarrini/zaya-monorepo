...
const GET = async ({
  ...
  const dataset = url.searchParams.get("dataset") || "sentences";
  ...
  try {
    const datasetInfo = AVAILABLE_DATASETS[dataset];
    if (!datasetInfo) {
      return json(
        {
          success: false,
    ...
    const errorMessage = "Failed to load sentences data";
    return json(
      {
        success: false,
        error: errorMessage,
...
function parseSentencesArray(jsonArray) {
  const sentences = [];
...
function parseSentencesObject(jsonObject) {
  const sentences = [];
...
function processSentences(sentences, page, pageSize, search, dataset, datasetInfo) {
  let filteredSentences = sentences;
  if (search) {
    const searchLower = search.toLowerCase();
    filteredSentences = sentences.filter(
      (sentence) => Object.entries(sentence).some(
        ([key, value]) => key !== "id" && typeof value === "string" && value.toLowerCase().includes(searchLower)
      )
    );
  }
  ...
  sentences.forEach((sentence) => {
    Object.keys(sentence).forEach((key) => {
      if (key !== "id" && sentence[key] && typeof sentence[key] === "string") {
