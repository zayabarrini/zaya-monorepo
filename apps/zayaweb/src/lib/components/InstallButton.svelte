<script>
  ...
  async function installPWA() {
    if (deferredPrompt) {
      try {
        const choiceResult = await deferredPrompt.prompt(); // Mostra o prompt de instalação
        console.log(
          "Escolha do usuário:",
        ...
        if (choiceResult.outcome === "accepted") {
          ...
          installed = true; // Mostra a notificação de sucesso
          // setTimeout(() => {
          //   const notification = document.querySelector(".notification");
          //   notification.classList.add("fade-out");
          //   setTimeout(() => (installed = false), 500); // Remove a notificação após a animação
          // }, 4500); // Inicia a animação de saída após 4.5 segundos
        } else {
          console.log("Usuário recusou a instalação");
        }
      } catch (error) {
        console.error(
          "Erro ao exibir o prompt de instalação:",
          error
        );
      } finally {
        deferredPrompt = null; // Limpa o evento
        showButton = false; // Esconde o botão após a escolha do usuário
      }
    }
  }
</script>
